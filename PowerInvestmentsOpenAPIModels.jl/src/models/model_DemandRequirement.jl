...
"""
Base.@kwdef struct DemandRequirement <: APIModel
    id::Int64
    name::String
...
function _decode(::Type{DemandRequirement}, _openapi_raw, _openapi_validate::Bool)
    _openapi_validate && _validate_schema(
        _SPEC,
        (
            resource="https://openapi.invalid/schema/external-bf6af3e70f4224d59935.json",
            pointer="",
        ),
        _openapi_raw,
        "decoding DemandRequirement";
        direction=:neutral,
    )
    ...
    return DemandRequirement(;
        ...
        growth_rate=_openapi_field_growth_rate,
        new_demand_mw=_openapi_field_new_demand_mw,
        new_construction_year=_openapi_field_new_construction_year,
...
function _form_fields(_openapi_value::DemandRequirement)
    ...
    _openapi_value.growth_rate isa Absent ||
        push!(_openapi_output, "growth_rate" => _openapi_value.growth_rate)
    _openapi_value.new_demand_mw isa Absent ||
        push!(_openapi_output, "new_demand_mw" => _openapi_value.new_demand_mw)
    _openapi_value.new_construction_year isa Absent || push!(
        _openapi_output,
        "new_construction_year" => _openapi_value.new_construction_year,
    )
    _openapi_value.region isa Absent ||
        push!(_openapi_output, "region" => _openapi_value.region)
    _openapi_value.value_of_lost_load isa Absent ||
        push!(_openapi_output, "value_of_lost_load" => _openapi_value.value_of_lost_load)
