...
**`max_demand_delay`** | **`Float64`** | Maximum number of minutes that demand can be deferred or delayed (minutes). Units: min. | [optional] [default to nothing]
**`max_demand_advance`** | **`Float64`** | Maximum number of minutes that demand can be scheduled in advance of the original schedule (minutes). Units: min. | [optional] [default to nothing]
