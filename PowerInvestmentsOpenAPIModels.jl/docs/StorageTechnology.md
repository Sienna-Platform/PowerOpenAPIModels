...
**`capacity_limits_charge`** | **`Union{Absent,Nothing,StorageTechnologyCapacityLimitsCharge}`** | Allowable installed charging capacity: a single bound applied to all capacity, or a mapping from a stringified float key to the bound at that key. Units: MW. | [optional]
**`capacity_limits_discharge`** | **`Union{Absent,Nothing,StorageTechnologyCapacityLimitsDischarge}`** | Allowable installed discharging capacity: a single bound applied to all capacity, or a mapping from a stringified float key to the bound at that key. Units: MW. | [optional]
**`capacity_limits_energy`** | **`Union{Absent,Nothing,StorageTechnologyCapacityLimitsEnergy}`** | Allowable installed energy capacity: a single bound applied to all capacity, or a mapping from a stringified float key to the bound at that key. Units: MWh. | [optional]
**`capital_costs`** | **`Union{Absent,Nothing,StorageCapitalCost}`** | Capital and interconnection cost for a storage technology's charge, discharge, and energy capacity (charge/discharge in USD/MW, energy in USD/MWh). | [optional]
