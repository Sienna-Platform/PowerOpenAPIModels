...
**`eligible_generators`** | **`Vector{String}`** | Names of individual generation units mapped to a technology that are eligible for retirement. | [default to nothing]
...
**`retirement_cost`** | [**`*ValueCurve`**](ValueCurve.md) |  | [default to nothing]
