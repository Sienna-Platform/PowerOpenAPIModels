"""
    KauraPLL

Parameters of a Phase-Locked Loop (PLL) based on 'Operation of a phase locked loop system under distorted utility conditions' by Vikram Kaura, and Vladimir Blasko

  - `omega_lp`: PLL low-pass filter frequency
  - `kp_pll`: PLL proportional gain
  - `ki_pll`: PLL integral gain
"""
Base.@kwdef struct KauraPLL <: APIModel
    omega_lp::Float64
    kp_pll::Float64
    ki_pll::Float64
    additional_properties::Dict{String, Any} = Dict{String, Any}()
end
_decode(::Type{KauraPLL}, value) = _decode(KauraPLL, value, true)
function _decode(::Type{KauraPLL}, _openapi_raw, _openapi_validate::Bool)
    _openapi_validate && _validate_schema(
        _SPEC,
        (
            resource="https://openapi.invalid/schema/external-89aa9f8e95ed696e68f0.json",
            pointer="",
        ),
        _openapi_raw,
        "decoding KauraPLL";
        direction=:neutral,
    )
    _openapi_object = _object(_openapi_raw, "KauraPLL")
    _openapi_field_omega_lp = _decode(
        Float64,
        _required(_openapi_object, "omega_lp", "KauraPLL"),
        _openapi_validate,
    )
    _openapi_field_kp_pll = _decode(
        Float64,
        _required(_openapi_object, "kp_pll", "KauraPLL"),
        _openapi_validate,
    )
    _openapi_field_ki_pll = _decode(
        Float64,
        _required(_openapi_object, "ki_pll", "KauraPLL"),
        _openapi_validate,
    )
    _openapi_additional_properties = Dict{String, Any}()
    for (_openapi_key, _openapi_item) in _openapi_object
        String(_openapi_key) in ("omega_lp", "kp_pll", "ki_pll") && continue
        _openapi_additional_properties[String(_openapi_key)] =
            _decode(Any, _openapi_item, _openapi_validate)
    end
    return KauraPLL(;
        omega_lp=_openapi_field_omega_lp,
        kp_pll=_openapi_field_kp_pll,
        ki_pll=_openapi_field_ki_pll,
        additional_properties=_openapi_additional_properties,
    )
end
function _encode(_openapi_value::KauraPLL)
    _openapi_output = JSON.Object{String, Any}()
    _openapi_value.omega_lp isa Absent ||
        (_openapi_output["omega_lp"] = _encode(_openapi_value.omega_lp))
    _openapi_value.kp_pll isa Absent ||
        (_openapi_output["kp_pll"] = _encode(_openapi_value.kp_pll))
    _openapi_value.ki_pll isa Absent ||
        (_openapi_output["ki_pll"] = _encode(_openapi_value.ki_pll))
    for (_openapi_key, _openapi_item) in _openapi_value.additional_properties
        haskey(_openapi_output, _openapi_key) && throw(
            ArgumentError(
                "additional property conflicts with declared field: " * _openapi_key,
            ),
        )
        _openapi_output[_openapi_key] = _encode(_openapi_item)
    end
    return _validate_schema(
        _SPEC,
        (
            resource="https://openapi.invalid/schema/external-89aa9f8e95ed696e68f0.json",
            pointer="",
        ),
        _openapi_output,
        "encoding KauraPLL";
        direction=:neutral,
    )
end

function _form_fields(_openapi_value::KauraPLL)
    _openapi_output = Pair{String, Any}[]
    _openapi_value.omega_lp isa Absent ||
        push!(_openapi_output, "omega_lp" => _openapi_value.omega_lp)
    _openapi_value.kp_pll isa Absent ||
        push!(_openapi_output, "kp_pll" => _openapi_value.kp_pll)
    _openapi_value.ki_pll isa Absent ||
        push!(_openapi_output, "ki_pll" => _openapi_value.ki_pll)
    append!(_openapi_output, collect(_openapi_value.additional_properties))
    return _openapi_output
end
