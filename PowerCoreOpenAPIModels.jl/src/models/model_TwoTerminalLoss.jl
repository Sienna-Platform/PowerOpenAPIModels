...
"""
struct TwoTerminalLoss <: OneOfAPIModel
...
function _decode(::Type{TwoTerminalLoss}, value, _openapi_validate::Bool)
    _openapi_validate && _validate_schema(
        ...
        (
            ...
            pointer="/components/schemas/TwoTerminalLoss",
        ),
        value,
        "decoding TwoTerminalLoss";
        direction=:neutral,
    )
    ...
    tag isa Absent ||
        ...
        throw(DecodeError("discriminator value must be a string for TwoTerminalLoss"))
    selected = get(
        Dict(
            "INCREMENTAL" => (
                IncrementalCurve,
    ...
    selected === nothing &&
        throw(DecodeError("unknown discriminator value $(repr(tag)) for TwoTerminalLoss"))
    !_openapi_validate ||
        _schema_valid(_SPEC, selected[2], value; direction=:neutral) ||
        throw(
            DecodeError(
                "discriminator-selected schema did not validate for TwoTerminalLoss",
            ),
        )
