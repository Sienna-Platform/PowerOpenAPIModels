...
"""
struct EmissionsDataEmissionRate <: OneOfAPIModel
    value::Union{
        AverageRateCurve,
        IncrementalCurve,
        InputOutputCurve,
...
_decode(::Type{EmissionsDataEmissionRate}, value) =
    _decode(EmissionsDataEmissionRate, value, true)
function _decode(::Type{EmissionsDataEmissionRate}, value, _openapi_validate::Bool)
    _openapi_validate && _validate_schema(
        ...
        (
            ...
            pointer="/components/schemas/EmissionsData/properties/emission_rate",
        ),
        value,
        "decoding EmissionsDataEmissionRate";
        direction=:neutral,
    )
    ...
    tag isa Absent ||
        ...
        throw(
            DecodeError(
                "discriminator value must be a string for EmissionsDataEmissionRate",
            ),
        )
    selected = get(
        Dict(
    ...
    selected === nothing && throw(
        DecodeError(
            "unknown discriminator value $(repr(tag)) for EmissionsDataEmissionRate",
        ),
    )
    !_openapi_validate ||
        ...
        throw(
            DecodeError(
                "discriminator-selected schema did not validate for EmissionsDataEmissionRate",
            ),
        )
...
function _encode(value::EmissionsDataEmissionRate)
    ...
    return _validate_schema(
        ...
        output,
        "encoding EmissionsDataEmissionRate";
        direction=:neutral,
    )
end
