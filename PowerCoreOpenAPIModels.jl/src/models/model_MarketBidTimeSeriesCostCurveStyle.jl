"""
    MarketBidTimeSeriesCostCurveStyle
...
"""
struct MarketBidTimeSeriesCostCurveStyle <: EnumAPIModel
    value::Int64
    function MarketBidTimeSeriesCostCurveStyle(value::Int64)
        value in (0, 1, 2) || throw(
            ArgumentError("invalid MarketBidTimeSeriesCostCurveStyle value $(repr(value))"),
        )
        return new(value)
    end
end
_decode(::Type{MarketBidTimeSeriesCostCurveStyle}, value) =
    _decode(MarketBidTimeSeriesCostCurveStyle, value, true)
function _decode(::Type{MarketBidTimeSeriesCostCurveStyle}, value, _openapi_validate::Bool)
    _openapi_validate && _validate_schema(
        ...
        (
            ...
            pointer="/components/schemas/MarketBidTimeSeriesCost/properties/curve_style",
        ),
        value,
        "decoding MarketBidTimeSeriesCostCurveStyle";
        direction=:neutral,
    )
...
function _encode(value::MarketBidTimeSeriesCostCurveStyle)
    ...
    return _validate_schema(
        ...
        output,
        "encoding MarketBidTimeSeriesCostCurveStyle";
        direction=:neutral,
    )
end
