# This file was generated by the Julia OpenAPI Code Generator
# Do not modify this file directly. Modify the OpenAPI specification instead.


@doc raw"""MarketBidTimeSeriesCost
Cost representation for time-varying market bids of energy and ancillary services. The static counterpart is MarketBidCost.

    MarketBidTimeSeriesCost(;
        cost_type="MARKET_BID_TIME_SERIES",
        minimum_energy_offer=nothing,
        start_up_association_id=nothing,
        shut_down=nothing,
        incremental_offer_curves=nothing,
        decremental_offer_curves=nothing,
        ancillary_service_offers=nothing,
        incremental_slope=false,
        decremental_slope=false,
        curve_style=0,
        curve_multistep=0,
    )

    - cost_type::String
    - minimum_energy_offer::TimeSeriesInputOutputCurve
    - start_up_association_id::Int64 : Store-minted id of a time series of three-stage (hot, warm, cold) start-up costs.
    - shut_down::TimeSeriesInputOutputCurve
    - incremental_offer_curves::CostCurve
    - decremental_offer_curves::CostCurve
    - ancillary_service_offers::Vector{Int64} : IDs of the ancillary service components that this bid offers into.
    - incremental_slope::Bool : Linear-interpolation flag for the incremental offer curves; false (default) is the step interpretation. Mutually exclusive with block groups on the same curve.
    - decremental_slope::Bool : Linear-interpolation flag for the decremental offer curves; false (default) is the step interpretation. Mutually exclusive with block groups on the same curve.
    - curve_style::Int64 : Curve-clearing style for the bid: 0 &#x3D; VARIABLE (default; continuous quantity with one or more segments); 1 &#x3D; FIXED (all-or-nothing block with a single segment). FIXED is mutually exclusive with incremental_slope/decremental_slope and requires a single-segment offer curve.
    - curve_multistep::Int64 : Multi-step block indicator for the bid: 0 &#x3D; SINGLE_STEP (default; each step of the bid clears independently); 1 &#x3D; MULTI_STEP (the bid must be awarded as one block across every step it covers). Counted in model steps so it applies at any resolution. Independent of curve_style: curve_style is the quantity structure, curve_multistep is the time structure, and they compose.
"""
Base.@kwdef mutable struct MarketBidTimeSeriesCost <: OpenAPI.APIModel
    cost_type::Union{Nothing, String} = "MARKET_BID_TIME_SERIES"
    minimum_energy_offer = nothing # spec type: Union{ Nothing, TimeSeriesInputOutputCurve }
    start_up_association_id::Union{Nothing, Int64} = nothing
    shut_down = nothing # spec type: Union{ Nothing, TimeSeriesInputOutputCurve }
    incremental_offer_curves = nothing # spec type: Union{ Nothing, CostCurve }
    decremental_offer_curves = nothing # spec type: Union{ Nothing, CostCurve }
    ancillary_service_offers::Union{Nothing, Vector{Int64}} = nothing
    incremental_slope::Union{Nothing, Bool} = false
    decremental_slope::Union{Nothing, Bool} = false
    curve_style::Union{Nothing, Int64} = 0
    curve_multistep::Union{Nothing, Int64} = 0

    function MarketBidTimeSeriesCost(cost_type, minimum_energy_offer, start_up_association_id, shut_down, incremental_offer_curves, decremental_offer_curves, ancillary_service_offers, incremental_slope, decremental_slope, curve_style, curve_multistep, )
        o = new(cost_type, minimum_energy_offer, start_up_association_id, shut_down, incremental_offer_curves, decremental_offer_curves, ancillary_service_offers, incremental_slope, decremental_slope, curve_style, curve_multistep, )
        OpenAPI.validate_properties(o)
        return o
    end
end # type MarketBidTimeSeriesCost

const _property_types_MarketBidTimeSeriesCost = Dict{Symbol,Type}(Symbol("cost_type")=>Union{Nothing, String}, Symbol("minimum_energy_offer")=>Union{Nothing, TimeSeriesInputOutputCurve}, Symbol("start_up_association_id")=>Union{Nothing, Int64}, Symbol("shut_down")=>Union{Nothing, TimeSeriesInputOutputCurve}, Symbol("incremental_offer_curves")=>Union{Nothing, CostCurve}, Symbol("decremental_offer_curves")=>Union{Nothing, CostCurve}, Symbol("ancillary_service_offers")=>Union{Nothing, Vector{Int64}}, Symbol("incremental_slope")=>Union{Nothing, Bool}, Symbol("decremental_slope")=>Union{Nothing, Bool}, Symbol("curve_style")=>Union{Nothing, Int64}, Symbol("curve_multistep")=>Union{Nothing, Int64}, )
OpenAPI.property_type(::Type{ MarketBidTimeSeriesCost }, name::Symbol) = _property_types_MarketBidTimeSeriesCost[name]

function OpenAPI.check_required(o::MarketBidTimeSeriesCost)
    o.cost_type === nothing && (return false)
    o.minimum_energy_offer === nothing && (return false)
    o.start_up_association_id === nothing && (return false)
    o.shut_down === nothing && (return false)
    o.incremental_offer_curves === nothing && (return false)
    o.decremental_offer_curves === nothing && (return false)
    o.ancillary_service_offers === nothing && (return false)
    true
end

function OpenAPI.validate_properties(o::MarketBidTimeSeriesCost)
    OpenAPI.validate_property(MarketBidTimeSeriesCost, Symbol("cost_type"), o.cost_type)
    OpenAPI.validate_property(MarketBidTimeSeriesCost, Symbol("minimum_energy_offer"), o.minimum_energy_offer)
    OpenAPI.validate_property(MarketBidTimeSeriesCost, Symbol("start_up_association_id"), o.start_up_association_id)
    OpenAPI.validate_property(MarketBidTimeSeriesCost, Symbol("shut_down"), o.shut_down)
    OpenAPI.validate_property(MarketBidTimeSeriesCost, Symbol("incremental_offer_curves"), o.incremental_offer_curves)
    OpenAPI.validate_property(MarketBidTimeSeriesCost, Symbol("decremental_offer_curves"), o.decremental_offer_curves)
    OpenAPI.validate_property(MarketBidTimeSeriesCost, Symbol("ancillary_service_offers"), o.ancillary_service_offers)
    OpenAPI.validate_property(MarketBidTimeSeriesCost, Symbol("incremental_slope"), o.incremental_slope)
    OpenAPI.validate_property(MarketBidTimeSeriesCost, Symbol("decremental_slope"), o.decremental_slope)
    OpenAPI.validate_property(MarketBidTimeSeriesCost, Symbol("curve_style"), o.curve_style)
    OpenAPI.validate_property(MarketBidTimeSeriesCost, Symbol("curve_multistep"), o.curve_multistep)
end

function OpenAPI.validate_property(::Type{ MarketBidTimeSeriesCost }, name::Symbol, val)

    if name === Symbol("cost_type")
        OpenAPI.validate_param(name, "MarketBidTimeSeriesCost", :enum, val, ["MARKET_BID_TIME_SERIES"])
    end










    if name === Symbol("curve_style")
        OpenAPI.validate_param(name, "MarketBidTimeSeriesCost", :enum, val, [0, 1])
    end


    if name === Symbol("curve_multistep")
        OpenAPI.validate_param(name, "MarketBidTimeSeriesCost", :enum, val, [0, 1])
    end

end
