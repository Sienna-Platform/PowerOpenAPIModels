# This file was generated by the Julia OpenAPI Code Generator
# Do not modify this file directly. Modify the OpenAPI specification instead.


@doc raw"""FromTo_ToFrom

    FromToToFrom(;
        from_to=nothing,
        to_from=nothing,
    )

    - from_to::Float64
    - to_from::Float64
"""
Base.@kwdef mutable struct FromToToFrom <: OpenAPI.APIModel
    from_to::Union{Nothing, Float64} = nothing
    to_from::Union{Nothing, Float64} = nothing

    function FromToToFrom(from_to, to_from, )
        o = new(from_to, to_from, )
        OpenAPI.validate_properties(o)
        return o
    end
end # type FromToToFrom

const _property_types_FromToToFrom = Dict{Symbol,Type}(Symbol("from_to")=>Union{Nothing, Float64}, Symbol("to_from")=>Union{Nothing, Float64}, )
OpenAPI.property_type(::Type{ FromToToFrom }, name::Symbol) = _property_types_FromToToFrom[name]

function OpenAPI.check_required(o::FromToToFrom)
    true
end

function OpenAPI.validate_properties(o::FromToToFrom)
    OpenAPI.validate_property(FromToToFrom, Symbol("from_to"), o.from_to)
    OpenAPI.validate_property(FromToToFrom, Symbol("to_from"), o.to_from)
end

function OpenAPI.validate_property(::Type{ FromToToFrom }, name::Symbol, val)


end
