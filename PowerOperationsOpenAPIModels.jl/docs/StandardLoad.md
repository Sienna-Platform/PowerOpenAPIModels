...
**`conformity`** | **`Union{Absent,Nothing,StandardLoadConformity}`** | Indicates whether the specified load is conforming or non-conforming. | [optional]
...
**`power_units`** | **`StandardLoadPowerUnits`** | Unit basis for this component's power-family fields (active/reactive/apparent power, ratings, limits, ramp rates). COMPONENT_BASE: per unit on this component's own base_power. NATURAL_UNITS: the field's physical unit. | [required]
