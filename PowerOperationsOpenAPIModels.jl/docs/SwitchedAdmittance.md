...
**`Y_increase`** | [**`Vector{ComplexNumber}`**](ComplexNumber.md) | Vector with admittance increment step for each adjustable shunt block. For example, &#x60;Y_increase[2]&#x60; is the complex admittance increment for each step at block 2. Units: per admittance_units — NATURAL_UNITS: S, DEVICE_MVAR: MVAr . | [optional] [default to nothing]
