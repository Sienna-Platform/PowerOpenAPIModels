...
**`control_mode`** | **`Union{Absent,FACTSControlDeviceControlMode,Nothing}`** | Control mode. Used to describe the behavior of the control device. | [optional]
**`voltage_setpoint_units`** | **`Union{Absent,Nothing,VoltageUnitBasis}`** | Unit basis for voltage_setpoint. COMPONENT_BASE (pu on the bus base voltage) is PSS/E RAW native (VSET). | [optional]
**`voltage_setpoint`** | **`Float64`** | Voltage setpoint at the regulated bus: the remote regulated bus when one is set, otherwise the sending end bus (PSS/E VSET). Units: per voltage_setpoint_units — NATURAL_UNITS: kV, COMPONENT_BASE: pu . | [required]
...
**`reactive_power_required`** | **`Union{Absent,Float64,Nothing}`** | Delivered reactive power after a solve, written by the power flow (an output, not parsed from input). Units: per power_units — NATURAL_UNITS: MVAr, COMPONENT_BASE: pu . | [optional]
...
**`remote_regulated_bus_id`** | **`Union{Absent,Union{Int64,Nothing}}`** | ID of the bus whose voltage this device regulates when that bus is not its sending end bus (PSS/E FCREG). Null means the sending end bus; a value equal to that bus is invalid. | [optional]
