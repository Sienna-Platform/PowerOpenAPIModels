...
**`voltage_setpoint_units`** | **`String`** | Unit basis for voltage_setpoint. DEVICE_BASE (pu on the bus base voltage) is PSS/E RAW native (VSET). | [optional] [default to "DEVICE_BASE"]
**`voltage_setpoint`** | **`Float64`** | Voltage setpoint at the sending end bus in kV, it has to be a &#x60;PV&#x60; bus. Units: kV. Units: per voltage_setpoint_units — NATURAL_UNITS: kV, DEVICE_BASE: pu . | [default to nothing]
**`max_shunt_current`** | **`Float64`** | Maximum shunt current at the sending end bus; entered at unity voltage. Units: MVA. | [default to nothing]
