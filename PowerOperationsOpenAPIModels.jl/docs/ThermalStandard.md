...
**`must_run`** | **`Union{Absent,Bool,Nothing}`** | Set to `true` if the unit is must run. | [optional]
...
**`time_at_status`** | **`Union{Absent,Float64,Nothing}`** | Time the generator has been on or off, as indicated by `status`. Units: min. | [optional]
