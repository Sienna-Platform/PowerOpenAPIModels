...
An energy storage device modeled as a generic reservoir, suitable for charging and discharging with average efficiency losses across storage types and chemistries. For pumped hydro, see HydroPumpTurbine and HydroReservoir instead.
  ...
  - `self_discharge`: Self-discharge (leakage) as a fraction of stored energy lost per minute, modeled as E[t] = (1 - self_discharge * dt) * E[t-1]; dt on the same minutes basis. Units: 1/min.
  - `standing_loss`: Constant standing-loss power drawn by the storage system: reduces effective charging power and increases power drawn when discharging. Units: per power_units — NATURAL_UNITS: MW, COMPONENT_BASE: pu .
  - `storage_capacity`: Maximum storage capacity, e.g. MWh for batteries or liters for hydrogen. Divided by base_power gives an approximate duration at unity power factor. Units: per energy_units — MWH: MWh, MWMIN: MWmin .
  - `storage_level_limits`: Minimum and maximum allowable storage levels [0, 1], usable to model derates or state-of-charge restrictions on cycling.
...
function _decode(::Type{EnergyReservoirStorage}, _openapi_raw, _openapi_validate::Bool)
    _openapi_validate && _validate_schema(
        ...
        (
            ...
            pointer="/components/schemas/EnergyReservoirStorage",
        ),
        _openapi_raw,
