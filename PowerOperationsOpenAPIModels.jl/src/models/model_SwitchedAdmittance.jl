...
A switched admittance with discrete steps, most often used in power flow studies. Total admittance is number_engaged * Y_increase, unless solved_admittance is set, in which case that value is the effective admittance.
  ...
  - `y_increase`: Admittance increment for each adjustable shunt block, e.g. Y_increase[2] is the increment at block 2. Units: per admittance_units — NATURAL_UNITS: S, COMPONENT_MVAR: MVAr .
  ...
  - `number_engaged`: Number of steps currently engaged (switched in) for each adjustable shunt block; cannot exceed number_of_steps for that block.
  - `number_of_steps`: Number of available steps for admittance increment at each adjustable shunt block.
