...
function _decode(::Type{HydroPowerPlant}, _openapi_raw, _openapi_validate::Bool)
    _openapi_validate && _validate_schema(
        _SPEC,
        (
            resource="https://openapi.invalid/schema/external-c6674402a78cc2331e8c.json",
            pointer="",
        ),
        _openapi_raw,
    ...
    _openapi_field_id = _decode(
        Int64,
        _required(_openapi_object, "id", "HydroPowerPlant"),
        _openapi_validate,
    )
    _openapi_field_name = _decode(
        String,
        _required(_openapi_object, "name", "HydroPowerPlant"),
        _openapi_validate,
    )
