...
A Non-Capacitor Line Commutated Converter (LCC)-HVDC transmission line. As implemented in PSS/E.
  ...
  - `compounding_resistance`: Compounding resistance controlling DC voltage: zero for inverter control, line resistance for rectifier control, else a fraction. Units: per parameter_units — NATURAL_UNITS: ohm, COMPONENT_BASE: pu .
  ...
  - `min_compounding_voltage`: Minimum compounded voltage. Not specified in per-unit. Used only in constant-gamma operation, where the AC transformer controls DC voltage. Units: per dc_voltage_units — NATURAL_UNITS: kV, COMPONENT_BASE: pu .
  ...
  - `power_mode`: Whether the LCC line is in power mode (true, setpoints in MW) or current mode (false, setpoints in Amperes).
  ...
  - `scheduled_dc_voltage`: Scheduled compounded DC voltage, by default the scheduled DC voltage at the inverter bus. Not specified in per-unit. Units: per dc_voltage_units — NATURAL_UNITS: kV, COMPONENT_BASE: pu .
  - `switch_mode_voltage`: Mode-switch DC voltage. Not specified in per-unit. If the line is in power mode and DC voltage falls below this value, it switches to current mode. Units: per dc_voltage_units — NATURAL_UNITS: kV, COMPONENT_BASE: pu .
  - `transfer_setpoint`: Desired power set point: MW if power_mode true, Amperes otherwise. Not per-unit. Positive: power consumed at the rectifier bus; negative: at the inverter bus. Units: per power_mode — true: MW, false: A .
"""
