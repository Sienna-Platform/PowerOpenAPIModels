...
A Non-Capacitor Line Commutated Converter (LCC)-HVDC transmission line. As implemented in PSS/E.
    ...
    TwoTerminalLCCLine(;
        ...
        loss=LossCurve(; power_units="NATURAL_UNITS", value_curve=LossValueCurve(InputOutputCurve(; curve_type="INPUT_OUTPUT", function_data=InputOutputCurveFunctionData(LinearFunctionData(; constant_term=0.0, function_type="LINEAR", proportional_term=0.0))))),
        base_power=nothing,
        power_units=nothing,
    )
    ...
    - scheduled_dc_voltage::Float64 : Scheduled compounded DC voltage. By default this parameter is the scheduled DC voltage in the inverter bus. This parameter must not be specified in per-unit. Units: kV. Units: per dc_voltage_units — NATURAL_UNITS: kV, COMPONENT_BASE: pu .
    - rectifier_bridges::Int64 : Number of bridges in series in the rectifier side.
    - rectifier_delay_angle_limits::MinMax
    - rectifier_rc::Float64 : Rectifier commutating transformer resistance per bridge. Units: per parameter_units — NATURAL_UNITS: ohm, COMPONENT_BASE: pu .
    - rectifier_xc::Float64 : Rectifier commutating transformer reactance per bridge. Units: per parameter_units — NATURAL_UNITS: ohm, COMPONENT_BASE: pu .
    ...
    - inverter_rc::Float64 : Inverter commutating transformer resistance per bridge. Units: per parameter_units — NATURAL_UNITS: ohm, COMPONENT_BASE: pu .
    - inverter_xc::Float64 : Inverter commutating transformer reactance per bridge. Units: per parameter_units — NATURAL_UNITS: ohm, COMPONENT_BASE: pu .
    ...
    - switch_mode_voltage::Float64 : Mode switch DC voltage. This parameter must not be added in per-unit. If LCC line is in power mode control, and DC voltage falls below this value, the line switch to current mode control. Units: kV. Units: per dc_voltage_units — NATURAL_UNITS: kV, COMPONENT_BASE: pu .
    ...
    - min_compounding_voltage::Float64 : Minimum compounded voltage. This parameter must not be added in per-unit. Only used in constant gamma operation (gamma_min &#x3D; gamma_max), and the AC transformer is used to control the DC voltage. Units: kV. Units: per dc_voltage_units — NATURAL_UNITS: kV, COMPONENT_BASE: pu .
    ...
    - rectifier_capacitor_reactance::Float64 : Commutating rectifier capacitor reactance magnitude per bridge. Units: per parameter_units — NATURAL_UNITS: ohm, COMPONENT_BASE: pu .
    ...
    - inverter_capacitor_reactance::Float64 : Commutating inverter capacitor reactance magnitude per bridge. Units: per parameter_units — NATURAL_UNITS: ohm, COMPONENT_BASE: pu .
...
Base.@kwdef mutable struct TwoTerminalLCCLine <: OpenAPI.APIModel
    ...
    loss = LossCurve(; power_units="NATURAL_UNITS", value_curve=LossValueCurve(InputOutputCurve(; curve_type="INPUT_OUTPUT", function_data=InputOutputCurveFunctionData(LinearFunctionData(; constant_term=0.0, function_type="LINEAR", proportional_term=0.0))))) # spec type: Union{ Nothing, LossCurve }
...
function OpenAPI.validate_property(::Type{ TwoTerminalLCCLine }, name::Symbol, val)
    ...
    if name === Symbol("parameter_units")
        OpenAPI.validate_param(name, "TwoTerminalLCCLine", :enum, val, ["NATURAL_UNITS", "COMPONENT_BASE"])
    end
