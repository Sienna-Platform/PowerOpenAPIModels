...
A renewable (wind or solar) generator whose output can be curtailed to satisfy system constraints, including proactive curtailment for reserves. For non-curtailable renewables, see RenewableNonDispatch.
  ...
  - `reactive_power_limits`: Minimum and maximum reactive power limits, used in some production cost simulations and in power flow when connected to a PV bus. Null if not applicable. Units: per power_units — NATURAL_UNITS: MVAr, COMPONENT_BASE: pu .
"""
Base.@kwdef struct RenewableDispatch <: APIModel
...
function _decode(::Type{RenewableDispatch}, _openapi_raw, _openapi_validate::Bool)
    _openapi_validate && _validate_schema(
        ...
        (
            ...
            pointer="/components/schemas/RenewableDispatch",
        ),
        _openapi_raw,
        "decoding RenewableDispatch";
        direction=:neutral,
    )
    _openapi_object = _object(_openapi_raw, "RenewableDispatch")
    _openapi_field_active_power = _decode(
        Float64,
        _required(_openapi_object, "active_power", "RenewableDispatch"),
        _openapi_validate,
    )
    _openapi_field_available = _decode(
        Bool,
        _required(_openapi_object, "available", "RenewableDispatch"),
        _openapi_validate,
    )
    _openapi_field_base_power = _decode(
        Float64,
        _required(_openapi_object, "base_power", "RenewableDispatch"),
        _openapi_validate,
    )
    _openapi_field_bus = _decode(
        Int64,
        _required(_openapi_object, "bus", "RenewableDispatch"),
        _openapi_validate,
    )
    ...
    _openapi_field_id = _decode(
        Int64,
        _required(_openapi_object, "id", "RenewableDispatch"),
        _openapi_validate,
    )
    _openapi_field_name = _decode(
        String,
        _required(_openapi_object, "name", "RenewableDispatch"),
        _openapi_validate,
    )
    _openapi_field_operation_cost = _decode(
        RenewableDispatchOperationCost,
        _required(_openapi_object, "operation_cost", "RenewableDispatch"),
        _openapi_validate,
    )
    _openapi_field_power_factor = _decode(
        Float64,
        _required(_openapi_object, "power_factor", "RenewableDispatch"),
        _openapi_validate,
    )
    _openapi_field_power_units = _decode(
        UnitSystem,
        _required(_openapi_object, "power_units", "RenewableDispatch"),
        _openapi_validate,
    )
    _openapi_field_prime_mover_type = _decode(
        PrimeMovers,
        _required(_openapi_object, "prime_mover_type", "RenewableDispatch"),
        _openapi_validate,
    )
    _openapi_field_rating = _decode(
        Float64,
        _required(_openapi_object, "rating", "RenewableDispatch"),
        _openapi_validate,
    )
    _openapi_field_reactive_power = _decode(
        Float64,
        _required(_openapi_object, "reactive_power", "RenewableDispatch"),
        _openapi_validate,
    )
...
function _encode(_openapi_value::RenewableDispatch)
    ...
    _openapi_value.power_factor isa Absent ||
        (_openapi_output["power_factor"] = _encode(_openapi_value.power_factor))
