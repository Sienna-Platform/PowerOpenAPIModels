...
struct ReserveDirection <: EnumAPIModel
    ...
    function ReserveDirection(value::String)
        value in ("UP", "DOWN", "SYMMETRIC") ||
            throw(ArgumentError("invalid ReserveDirection value $(repr(value))"))
        return new(value)
    end
end
...
function _decode(::Type{ReserveDirection}, value, _openapi_validate::Bool)
    _openapi_validate && _validate_schema(
        ...
        (
            ...
            pointer="/\$defs/ReserveDirection",
        ),
        value,
        "decoding ReserveDirection";
        direction=:neutral,
    )
    return ReserveDirection(_decode(String, value, false))
end
function _encode_unvalidated(value::ReserveDirection)
    output = _encode_unvalidated(value.value)
    return output
end
_encode(value::ReserveDirection) = _validate_schema(
    _SPEC,
    (
        resource="https://openapi.invalid/schema/external-98ed6538b1d3543b4530.json",
        pointer="/\$defs/ReserveDirection",
