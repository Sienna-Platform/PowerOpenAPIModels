...
"""
struct HydroReservoirHeadToVolumeFactor <: OneOfAPIModel
    value::Union{
        LinearFunctionData,
        PiecewiseLinearData,
        PiecewiseStepData,
...
_decode(::Type{HydroReservoirHeadToVolumeFactor}, value) =
    _decode(HydroReservoirHeadToVolumeFactor, value, true)
function _decode(::Type{HydroReservoirHeadToVolumeFactor}, value, _openapi_validate::Bool)
    _openapi_validate && _validate_schema(
        ...
        (
            ...
            pointer="/components/schemas/HydroReservoir/properties/head_to_volume_factor",
        ),
        value,
        "decoding HydroReservoirHeadToVolumeFactor";
        direction=:neutral,
    )
    ...
    tag isa Absent ||
        ...
        throw(
            DecodeError(
                "discriminator value must be a string for HydroReservoirHeadToVolumeFactor",
            ),
        )
    selected = get(
        Dict(
    ...
    selected === nothing && throw(
        DecodeError(
            "unknown discriminator value $(repr(tag)) for HydroReservoirHeadToVolumeFactor",
        ),
    )
    !_openapi_validate ||
        ...
        throw(
            DecodeError(
                "discriminator-selected schema did not validate for HydroReservoirHeadToVolumeFactor",
            ),
        )
...
function _encode(value::HydroReservoirHeadToVolumeFactor)
    ...
    return _validate_schema(
        ...
        output,
        "encoding HydroReservoirHeadToVolumeFactor";
        direction=:neutral,
    )
end
