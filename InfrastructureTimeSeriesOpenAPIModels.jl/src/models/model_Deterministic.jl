...
A deterministic forecast: one horizon-length array per window, windows stepping by interval. One of six canonical time series types. Records the association and metadata; dense values live in the store named by uri.
  ...
  - `time_series_type`: Discriminator, fixed to Deterministic for this schema.
...
function _decode(::Type{Deterministic}, _openapi_raw, _openapi_validate::Bool)
    ...
    _openapi_field_owner_id = _decode(
        Int64,
        _required(_openapi_object, "owner_id", "Deterministic"),
        _openapi_validate,
    )
