...
# exactly what the umbrella package is for. Typing `time_series_associations` concretely
# would otherwise make Core depend on TimeSeries while TimeSeries already depends on Core (a
# package cycle), and PowerCore itself cannot host this container: it depends on
# InfrastructureCore, not the reverse.
#
...
# `has_model_type`, `DocumentFormatError`) lives in
# `InfrastructureCoreOpenAPIModels.jl/src/document.jl`: every domain package's generated
# `register.jl` calls `register_model_type!` in `__init__`, unconditionally qualified to
# `InfrastructureCoreOpenAPIModels`, so the registry has to be reachable whenever any single
# domain package is loaded — including InfrastructureCore itself, the base of the whole
# chain — independent of whether this umbrella package is ever loaded. References to it below
